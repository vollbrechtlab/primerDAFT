...
def get_target_attrs(target,side,idx):
...
def get_offtarget_attrs(off_target,side,idx):
    primer_seq = data["result"]["pairs"][idx]["PRIMER_"+side.upper()]["SEQUENCE"]
    attr_names=["sseqid","sstart","send","strand"]
    json_names=["chr","start","end","strand"]
    cols = [x+"_"+side for x in side_cols] + target_cols
...
def spec_check():
	...
	for x in list(pd.unique(pot_targets["pair"])):
	    ...
	    for target in curr_targets.itertuples():
	        ...
	        target_dict["left"] = get_target_attrs(target,"left",x)
	        target_dict["right"] = get_target_attrs(target,"right",x)
	...
	for x in list(pd.unique(off_targets["pair"])):
	    ...
	    for off_target in curr_targets.itertuples():
	        ...
	        off_target_dict["left"] = get_offtarget_attrs(off_target,"left",x)
	        off_target_dict["right"] = get_offtarget_attrs(off_target,"right",x)
	        ...
	        off_target_dict["target_seq"] = pysam_fasta.fetch(region=off_target_dict["left"]["chr"]+":"+str(gen_start)+"-"+str(gen_end))
